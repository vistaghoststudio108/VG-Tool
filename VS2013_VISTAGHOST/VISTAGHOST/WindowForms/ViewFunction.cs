...
namespace Vistaghost.VISTAGHOST
{
    public partial class ViewFunction : Form
    {
        ...
        {
            if (this.dte2.ActiveDocument == null ||
                this.dte2.ActiveDocument.Selection == null)
            {
                return;
            }

            if (this.dte2.ActiveDocument.ProjectItem == null)
            {
                lblStatus.Text = "This file is out of project. Try again";
                return;
            }
            ...
            var fcm = dte2.ActiveDocument.ProjectItem.FileCodeModel;
        ...
        void UpdateLineNumber(int offsetLine)
        {
            foreach (var func in objList)
            {
                if (func.Line > activeLine)
                {
                    func.Line += offsetLine;
                }
            }
        }
        ...
        {
            var pointer = this.dte.ActiveDocument.Selection as TextSelection;
            pointer.GotoLine(objList[index].Line);
        }
        ...
        private void hif_OnSendHeaderInfo(List<LVFuncInfo> funcinfo)
        {
            ...
            foreach (var f in funcinfo)
            {
                ...
                if (vgOperations.ProcessStringForAddHeader(this.dte, objList[f.Index], out offsetLine))
                {
        ...
        private void ViewFunction_Load(object sender, EventArgs e)
        {
            ...
            this.Text = this.dte.ActiveDocument.FullName;
